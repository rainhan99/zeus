...
2. **Verify mode routing.** If `$ARGUMENTS` contains `mode-resolved=`, the command layer already routed — proceed with sequential execution. If it does NOT contain `mode-resolved=` (direct skill invocation without going through `/zeus:execute`), ask the user via `AskUserQuestion`: "选择执行模式 / Choose execution mode" with options `sequential` / `subagent`. If user picks subagent, hand off to `zeus:subagent-driven-development` with the same arguments. Never auto-select — the user always decides.
...
| "`mode-resolved=` is set but I'll ask about mode anyway, just in case." | The marker means the command layer already ran the dialogue. Re-asking adds friction and contradicts the just-made choice. Honor the marker. |
