...
> Note on forward references: this skill names sibling skills (`zeus:e2e-gate`, `zeus:session-init`, etc.) that are not delivered until SP4–SP7. SP1 landed the bootstrap. SP2 landed the three `zeus:kickoff-*` skills. SP3 landed `zeus:brainstorming`, `zeus:writing-plans`, and `zeus:decompose-large-projects`. Until the rest land, the remaining references read as a forward-looking map of the plugin's intended shape — useful as a roadmap and as the gatekeeper-routing lookup table.
