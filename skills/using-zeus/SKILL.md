...
> Note on forward references: this skill names sibling skills (`zeus:session-init`, `zeus:session-handoff`, etc.) that are not delivered until SP6–SP7. SP1 landed the bootstrap. SP2 landed the three `zeus:kickoff-*` skills. SP3 landed `zeus:brainstorming`, `zeus:writing-plans`, and `zeus:decompose-large-projects`. SP4 landed `zeus:executing-plans`, `zeus:test-driven-development`, `zeus:subagent-driven-development`, `zeus:dispatching-parallel-agents`, and `zeus:using-git-worktrees`. SP5 landed `zeus:verification-before-completion`, `zeus:e2e-gate`, `zeus:systematic-debugging`, `zeus:requesting-code-review`, and `zeus:receiving-code-review`. Until the rest land, the remaining references read as a forward-looking map of the plugin's intended shape — useful as a roadmap and as the gatekeeper-routing lookup table.
