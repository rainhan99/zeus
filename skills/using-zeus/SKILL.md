...
> Note on forward references: this skill names sibling skills that are not all delivered yet. SP1 landed the bootstrap. SP2 landed the three `zeus:kickoff-*` skills. SP3 landed `zeus:brainstorming`, `zeus:writing-plans`, and `zeus:decompose-large-projects`. SP4 landed `zeus:executing-plans`, `zeus:test-driven-development`, `zeus:subagent-driven-development`, `zeus:dispatching-parallel-agents`, and `zeus:using-git-worktrees`. SP5 landed `zeus:verification-before-completion`, `zeus:e2e-gate`, `zeus:systematic-debugging`, `zeus:requesting-code-review`, and `zeus:receiving-code-review`. SP6 landed `zeus:memory-management`, `zeus:session-init`, `zeus:long-task-continuity`, `zeus:session-handoff`, and `zeus:clean-state`. Until SP7 lands, the remaining references (`zeus:observability`, `zeus:finishing-a-development-branch`, `zeus:writing-skills`) read as a forward-looking map of the plugin's intended shape.
